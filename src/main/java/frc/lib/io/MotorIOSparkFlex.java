...
public class MotorIOSparkFlex extends MotorIO {
	 ...
	 */
	protected void updateMotorInputs(Inputs inputsToUpdate, SparkFlex motor) {
	...
	private void setIdleMode(SparkFlex spark, IdleMode idleMode) {
		SmartDashboard.putNumber("SPARK FLEX NEUTRAL MODE SET!!", Timer.getFPGATimestamp());
		threadPoolExecutor.submit(() -> {
	...
	public MotorIOSparkFlex(MotorIOSparkFlexConfig config) {
		...
		main = new SparkFlex(config.mainID, MotorType.kBrushless);
		...
		for (int i = 0; i < config.followerIDs.length; i++) {
			followers[i] = new SparkFlex(config.followerIDs[i], MotorType.kBrushless);
